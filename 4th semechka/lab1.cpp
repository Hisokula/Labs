...
В данной задаче требуется ввести N точек своими координатами (x,y). Затем требуется определить, существует ли выпуклая оболочка заданного множества точек. 
...
public:
    ...
    //void NewCoords(Spot x) { x_m = x.get_x(); y_m = x.get_y(); }
...
// написать функцию для нахождения расстояния, для того чтобы брались только самые дальные точки на одной прямой
...
//(y1-y2)*x3+(x2-x1)*y3+(x1*y2-x2*y1)
void Check(std::vector<Spot> hull)
{
    for (int i = 0; i < hull.size() - 2; i++)
    {
        if ((hull[i].get_y() - hull[i + 1].get_y()) * hull[i + 2].get_x() + (hull[i + 1].get_x() - hull[i].get_x()) * hull[i + 2].get_y() + (hull[i].get_x() * hull[i + 1].get_y() - hull[i + 1].get_x() * hull[i].get_y()))
        {
            hull.erase(hull.begin() + 1);
        }
    }
}
//sqrt(pow(hull[i + 1].get_x() - hull[i].get_x(), 2) + pow(hull[i + 1].get_y() - hull[i].get_y(), 2))
